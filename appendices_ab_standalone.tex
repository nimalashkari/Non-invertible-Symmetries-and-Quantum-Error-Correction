...
\subsection{Two choices of algebras}
...
\end{align}
We also have a right-invariant Haar measure $d\mu_R(g)$, related to the left Haar measure by the modular function $\Delta:G\to\mathbb R_+$ through right translations,
\begin{align}
    d\mu_L(xg)=\Delta(g)d\mu_L(x).
\end{align}
For unimodular groups, and in particular for compact groups, $\Delta(g)=1$ and the left and right Haar measures may be normalized to agree. We then simplify our notation by defining $\|f\|_p\equiv \|f\|_{p,L}$ and work with the Haar measure $\mu\equiv \mu_L=\mu_R$. To keep track of inversion, we introduce the antipode map $S:g\to g^{-1}$, which satisfies $S=S^{-1}$.

For simplicity, we will focus on compact groups, for which the Haar measure is finite. The $L^\infty(G)$ is the isometric dual of $L^1(G)$ via the Hölder pairing,
\begin{equation}
    \langle f, h \rangle \equiv \int_G f(x) h(x) \, d\mu(x)\ .
\end{equation}
For compact groups, we have continuous inclusions
\begin{align}\label{hierLp}
    L^\infty(G)\subseteq L^2(G)\subseteq L^1(G)
\end{align}
Note that $L^2(G)$ is self-dual, whereas $L^\infty(G)$ is the Holder dual of  $L^1(G)$ \footnote{In this note, we will be primarily concerned with finite groups where this hierarchy simplifies to $\mathbb{C}(G)\simeq L^1(G)\simeq L^2(G)\simeq L^\infty(G)$ as vector spaces.}.
...
\begin{align}
    &\text{Pointwise}: \qquad (f_1\cdot f_2)(g)=f_1(g)f_2(g)\nn\\
    &\text{Convolution}:\qquad (f_1*f_2)(g)=\int d\mu(h) \: f_1(h)f_2(h^{-1}g)
\end{align}
...
\paragraph{Algebra of observables $L^\infty(G)$:} Pointwise multiplication is manifestly commutative, and since $\|f_1\cdot f_2 \|_\infty\leq \|f_1\|_\infty\|f_2\|_\infty$, the vector space $L^\infty(G)$ is closed with respect to this multiplication, and hence an Abelian algebra. This Abelian algebra encodes the {\it geometry of group $G$} \footnote{For any measurable space $X$, we have an Abelian algebra $L^\infty(X)$, and a classical result establishes that to every Abelian algebra one can associate a measurable space. We will refer to this as classical Gelfand duality. In an analogy with classical mechanics that interprets $G$ as the phase space, functions $L^\infty(G)$ have the interpretation of classical observables.}. 

\paragraph{Algebra of states $L^1(G)$ (pre-dual):} Convolution multiplication results in an $L^1(G)$ because, by Young's convolution inequality, $\|f * g\|_1 \leq \|f\|_1 \|g\|_1$, ensuring closure \footnote{Note that $L^1(G)$ does not form an algebra under pointwise multiplication due to non-integrable singularities in the squared functions.}. In the left regular representation, these $L^1$ functions define bounded integral operators that smear the group action over the manifold.
...
\item {\bf Right convolution algebra:} We can also define right convolution through the right regular operators. If $\iota_1^R(f)=\sum_g f(g)U'_g$, then $\iota_1^R(f)\xi(x)=\int d\mu(g)\, f(g)\xi(xg)$. With this multiplication, we obtain a different Banach space $L_R^1(G)$, and we can show explicitly that all operators in $L^1(G)$ and $L^1_R(G)$ commute. In fact, they are isometric with the isomorphism established by the antipode map $S:g\to g^{-1}$ that swaps the left-convolution with the right-convolution.

\end{itemize}

\subsection{Embedding inside $B(L^2(G))$}
...
\end{align}
where $S$ is the antipode. Note that $U_g$ and $U'_g$ are unitary, and we can write
\begin{align}
    L^1(G)=\mathbb{C}\rtimes_{\iota_1} G
\end{align}
Note that $U_{g_1}U_{g_2}=U_{g_1g_2}$ implies that the multiplication of two functions $\delta_{g,g_1}$ and $\delta_{g,g_2}$ should result in $\delta_{g,g_1g_2}$ which fixes the multiplication to be convolution:
\begin{align}
 \delta_{g,g_1}*\delta_{g,g_2}=\delta_{g,g_1g_2}.   
\end{align}
Since $S$ acts as a unitary on $B(L^2(G))$ we also have a second representation for $L^\infty(G)$. We introduce the notation
\begin{align}
    &\iota^\pm_\infty(f)=\sum_g f(g) e_g^\pm, \qquad e_g^+=e_g,\qquad  e_g^-=\ket{g^{-1}}\bra{g^{-1}}\nn\\
    &\iota^\pm_1(f)=\sum_g f(g) U_g^\pm, \qquad U_g^+=U_g, \qquad U_g^-=U'_g\nn\\
    &\iota_1^\pm=S\circ \iota_1^\mp\circ S,\qquad \iota_\infty^\pm=S\circ \iota_\infty^\mp\circ S
\end{align}
and we have the relations
\begin{align}
    &U^\pm_g e_h^\pm=e^\pm_{g h}U_g^\pm,\qquad U^\mp_g e_h^\pm=e^\pm_{h g^{-1}} U_g^{\mp}
\end{align}

As we saw $L^1_R(G)$ is in the relative commutant of the inclusion $\iota_1$. 
In fact, we have
\begin{align}
    &\iota_1(L^1(G))'\simeq \iota_1^R(L^1_R(G))\nn\\
    &\iota_1(L^1(G))\vee \iota_1^R(L^1_R(G))\simeq
    \bigoplus_{r\in\hat G}B(\mH_r\otimes\mH_r^*)\ .
\end{align}
Furthermore, we have
\begin{align}
...
\begin{align}
    ...
    &\mH_\infty^\pm=P_\infty^\pm\mH\equiv \overline{\iota_\infty^\pm(f)\ket{1}}\ .
\end{align}
These are the projections onto the GNS subspaces associated with the corresponding subalgebras.
...
\end{align}
These Hilbert-space adjoints are useful linear maps between the two embedded copies of functions. They should not be confused with algebra isomorphisms in the non-Abelian case: the non-Abelian Fourier transform identifies the group algebra with $\oplus_{r\in\hat G}B(\mH_r)$, rather than with the commutative algebra $L^\infty(G)$.
...
\end{align}
Note that here $Z(L^1(G))$ is the algebra of class functions under the finite-group identification of $L^1(G)$ with the group algebra.
...
It is convenient to choose the tracial state on $B(L^2(G))$ and use the GNS representation described above.


This embedding combined with its dual, gives the conditional expectation
\begin{align}
(\iota_\infty\circ\iota_\infty^\dagger)(a)=\sum_g \braket{g|a|g}\ket{g}\bra{g}.
\end{align}
...
\begin{align}
    Z(\mathcal{L}(G))=\mathcal{L}(G)\cap \mathcal{L}(G)'
\end{align}
Note that for finite groups we have $\mathcal{L}(G)\simeq L^1(G)\simeq L^2(G)\simeq \mathbb{C}(G)$ as finite-dimensional vector spaces.
Applying the Haar average over $G$ projects $\mathcal L(G)$ to $Z(\mathcal L(G))$:
\begin{align}
    &\mathcal{E}(a)=\frac{1}{|G|}\sum_g U_g a U_g^\dagger\nn\\
    &\mathcal{E}:\mathcal L(G)\to Z(\mathcal L(G))\ .
\end{align}
...
The generalized Fourier transform serves as a mathematical bridge between these branches. Together with their coproducts, the commutative geometry of $L^\infty(G)$ and the non-commutative dynamics of $\mathcal{L}(G)$ fit into the locally compact quantum group framework.
...
The fusion rules of the representation category $\mathrm{Rep}(D_8)$ govern how tensor products of representations decompose: $V_i \otimes V_j \cong \bigoplus_k N_{ij}^k V_k$. Interestingly, these fusion rules can be derived from the class functions by looking at the center $Z(\mathcal{L}(D_8))$ as a vector space and using pointwise multiplication of characters.
...
\end{align}
This demonstrates how pointwise multiplication of characters encodes the fusion dynamics of $\mathrm{Rep}(D_8)$.
